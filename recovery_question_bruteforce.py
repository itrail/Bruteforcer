...
with open("password_list.txt") as f:
    list_of_answers = [line.rstrip() for line in f]
...
if attemps_per_one_IP * len(proxy_list) < len(list_of_answers):
    print(
        f"It's too much answers (`{len(list_of_answers)}`) to check for `{len(proxy_list)}` IPs! Incease the number of attemps per one IP or decrease the number of answers"
    )
else:
